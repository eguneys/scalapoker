...
class ScoreTest extends PokerTest {
  ...
  "straight" should {
    ...
    val hand4: Hand = "5h 6s 6h 7c 9s Ts 8d"

    "should find straight" in {
      hand.value must_== Straight(Six, "6s 5s 4s 3s 2s")
      hand2.value must_== Straight(Ten, "Ts 9s 8c 7s 6s")
      hand3.value must_== OnePair(Ace, "6s 6h Ts 8d 7c")
    }

    "should find inner straight" in {
      hand4.value must_== Straight(Ten, "Ts 9s 8d 7c 6s")
